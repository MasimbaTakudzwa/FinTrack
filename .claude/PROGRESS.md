...
**Last updated:** 2026-04-22 — Session 003 (checkpoint 13 — post-Sprint-4 bug triage + arbitrary-asset add + AssetDetail rebuild + in-app NotificationCenter)
**Active sprint:** Sprint 4 ✅ (4A/B/C/D done) + post-launch polish pass
**Overall status:** 🟢 Sprints 1, 2, 3, 4 complete. Post-launch session closed 8 user-reported items: 3 bugs (watchlist delete, alert delete, empty top losers), 1 major feature (arbitrary-asset lookup+add), 3 AssetDetail upgrades (timeframe toggle, measurement tool, performance/stats/alerts panels filling blank space), and 1 UX deliverable (in-app NotificationCenter in the Header — addresses the "alerts show up as Terminal" issue for dev mode and gives persistent history in prod)
...
2. [ ] **Sprint 5 — Packaging & distribution**: PyInstaller-freeze the sidecar, wire it into Tauri as `external_bin`, GH Actions macOS + Windows matrix, code-sign both, ship v0.1.0. Bundled `.app` will also fix the "Terminal" notification attribution for free.
