...
**Last updated:** 2026-04-22 — Session 003 (checkpoint 12 — Sprint 4D complete: price alerts + desktop notifications end-to-end)
**Active sprint:** Sprint 4 — News, Watchlists & Desktop Alerts (4A ✅, 4B ✅, 4C ✅, 4D ✅) — **SPRINT 4 COMPLETE**
**Overall status:** 🟢 Sprints 1, 2 & 3 complete; Sprint 4 fully done — price alerts fire native desktop notifications via the Tauri plugin + polling handshake, `/alerts` page manages the history, AssetDetail has a Create-alert modal
...
1. [x] **Sprint 4A/B/C/D** — all four milestones complete. **Sprint 4 done.**
2. [ ] **Live GUI smoke for Sprint 4 end-to-end**: `pnpm tauri dev`, visit `/watchlists` (4C smoke still pending), then open an asset page, click "Create alert", set a threshold just above the current close for an asset that moves a lot (crypto or a volatile stock). Wait for the `check_price_alerts` job to fire (≤1 min by default) — a native OS notification should appear within ≤30 s after. Open `/alerts` and confirm the row shows Triggered + an amber tint. Reset the alert; verify the status flips back to Armed. Pause/Resume and Delete should all work with no page reload.
3. [ ] **Sprint 5 — Packaging & distribution**: PyInstaller-freeze the sidecar, wire it into Tauri as `external_bin`, set up GitHub Actions macOS + Windows matrix, code-sign both, ship v0.1.0.
