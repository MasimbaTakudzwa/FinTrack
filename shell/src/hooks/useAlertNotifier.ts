...
async function processPending(permissionOk: boolean): Promise<void> {
  const list = await listPendingAlertNotifications();
    ...
    const tick = (): void => {
      ...
      void (async () => {
        try {
          ...
          await processPending(permissionRef.current);
        } catch (err) {
          // Network error or sidecar not up yet — swallow, retry next tick.
